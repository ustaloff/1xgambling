...
class SlotsLaunch_Notices {
	...
	public static function getInstance() {
		...
		if ( is_null( self::$_instance ) ) {
			self::$_instance = new SlotsLaunch_Notices();
		}
	...
	public static function add( $message, $type = '' ) {
		...
		$id       = 'slotsl-notice-' . ( count( $instance->notices ) + 1 );
		$type     = ! empty( $type ) ? 'notice-' . $type : '';
		$notice   = sprintf( '<div class="notice slotsl-notice %s" id="%s">%s</div>', $type, $id, wpautop( $message ) );
