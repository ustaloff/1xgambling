...
function aces_organization_single_shortcode_2( $atts ) {
	...
	if ( get_option( 'aces_casino_link_mask_slug') && $organization_external_link ) {
		
		$casino_mask_link_slug = esc_attr( get_post_meta( get_the_ID(), 'casino_mask_link', true ) );

		if ( $casino_mask_link_slug ) {

			if ( $_GET['m'] == $casino_mask_link_slug ){
				$redirect_link = esc_url( $organization_external_link );
				header('Location: ' . $redirect_link );
				die();
			}

			$organization_external_link = esc_url( get_the_permalink().'?m='.$casino_mask_link_slug );
	...
	<div class="space-organization-single-2 box-100 relative <?php if ( $dark_background ) { ?>dark-background<?php } ?>">
		...
		<div class="space-style-2-organization-header-elements box-100 relative">
					...
					<div class="space-organization-header-logo-title box-100 relative">
						<div class="space-organization-header-logo-box relative">
							...
							if ( wp_get_attachment_image($organization_logo_id) ) {
								echo wp_get_attachment_image( $organization_logo_id, 'mercury-135-135', "", array( "alt" => $post_title_attr ) );
							} ?>
							<?php if ($organization_overall_rating) { ?>
								<div class="space-organization-header-logo-rating absolute">
									<?php echo esc_html( number_format( (float)$organization_overall_rating, 1, '.', ',') ); ?> <i class="fas fa-star"></i>
								</div>
							<?php } ?>
						</div>
			...
			<div class="space-style-2-organization-header-right box-25 relative">
				<div class="space-organization-header-button text-center box-100 relative">
					...
					<a href="<?php echo esc_url( $organization_external_link ); ?>" title="<?php echo esc_attr( $button_title ); ?>" class="space-style-2-button" rel="nofollow" target="_blank"><?php echo esc_html( $button_title ); ?> <i class="fas fa-arrow-alt-circle-right"></i></a>
...
add_shortcode('aces-organization-2', 'aces_organization_single_shortcode_2');