...
function aces_unit_single_shortcode_2( $atts ) {
	...
	if ( get_option( 'aces_game_link_mask_slug') && $unit_external_link ) {
		
		$game_mask_link_slug = esc_attr( get_post_meta( $item_id, 'game_mask_link', true ) );

		if ( $game_mask_link_slug ) {

			if ( $_GET['m'] == $game_mask_link_slug ){
				$redirect_link = esc_url( $unit_external_link );
				header('Location: ' . $redirect_link );
				die();
			}

			$unit_external_link = esc_url( get_the_permalink().'?m='.$game_mask_link_slug );
