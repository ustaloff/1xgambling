...
function aces_offer_single_shortcode_1( $atts ) {
	...
	if ( get_option( 'aces_bonus_link_mask_slug') && $offer_external_link ) {
		...
		if ( $bonus_mask_link_slug ) {
			...
			if ( $_GET['m'] == $bonus_mask_link_slug ){
				$redirect_link = esc_url( $offer_external_link );
				header('Location: ' . $redirect_link );
				die();
			}

			$offer_external_link = esc_url( get_the_permalink().'?m='.$bonus_mask_link_slug );
