...
function aces_games_display_meta_box( $game ) {
	...
	if ( get_option( 'aces_game_link_mask_slug') ) {
		$game_mask_link = get_post_meta( $game->ID, 'game_mask_link', true );
	}
...
<?php if ( get_option( 'aces_game_link_mask_slug') ) { ?>

	<div class="components-base-control game_mask_link">
		<div class="components-base-control__field">
			<label class="components-base-control__label" for="game_mask_link-0"><?php esc_html_e( 'Custom slug to mask external URL', 'aces' ); ?></label>
			<input type="text" name="game_mask_link" id="game_mask_link-0" value="<?php echo esc_attr($game_mask_link); ?>" style="display: block; margin-bottom: 10px;" />
		</div>
	</div>
...
function aces_games_save_fields( $post_id ) {
        ...
        if ( get_option( 'aces_game_link_mask_slug') ) {
        	...
        	$game_mask_link = sanitize_text_field( $_POST['game_mask_link'] );
        	update_post_meta( $post_id, 'game_mask_link', $game_mask_link );
