...
function aces_bonuses_settings_init() {
    ...
    add_settings_field(
        'aces_bonus_link_mask_slug',
        esc_html__( 'Enable link masking slug', 'aces' ),
        'aces_bonus_link_mask_slug_callback',
        'aces_bonuses_tab',
        'aces_bonuses_tab_other_settings',
        array(
            'id' => 'aces_bonus_link_mask_slug', 
            'option_name' => 'aces_bonus_link_mask_slug'
        )  
    );
    register_setting( 'aces_bonuses_tab', 'aces_bonus_link_mask_slug', 'esc_attr');

    function aces_bonus_link_mask_slug_callback($args) {
        $option = get_option( 'aces_bonus_link_mask_slug' );
        $id = $args['id'];
        $option_name = $args['option_name'];
        ?>
        <input type="checkbox" name="aces_bonus_link_mask_slug" value="1" <?php checked(1, get_option('aces_bonus_link_mask_slug'), true); ?> />
        <?php
    }
