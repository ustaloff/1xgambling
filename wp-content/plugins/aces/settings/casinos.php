...
function aces_casinos_settings_init() {
    ...
    add_settings_field(
        'aces_rating_custom_value',
        esc_html__( 'Enable custom values', 'aces' ),
        'aces_rating_custom_value_callback',
        'aces_casinos_tab',
        'aces_casinos_tab_rating_titles',
        array(
            'id' => 'aces_rating_custom_value', 
            'option_name' => 'aces_rating_custom_value'
        )  
    );
    register_setting( 'aces_casinos_tab', 'aces_rating_custom_value', 'esc_attr');

    function aces_rating_custom_value_callback($args) {
        $option = get_option( 'aces_rating_custom_value' );
        $id = $args['id'];
        $option_name = $args['option_name'];
        ?>
        <input type="checkbox" name="aces_rating_custom_value" value="1" <?php checked(1, get_option('aces_rating_custom_value'), true); ?> />
        <?php
    }
    ...
    /*  --- Enable link masking slug ---  */

    add_settings_field(
        'aces_casino_link_mask_slug',
        esc_html__( 'Enable link masking slug', 'aces' ),
        'aces_casino_link_mask_slug_callback',
        'aces_casinos_tab',
        'aces_casinos_tab_other_settings',
        array(
            'id' => 'aces_casino_link_mask_slug', 
            'option_name' => 'aces_casino_link_mask_slug'
        )  
    );
    register_setting( 'aces_casinos_tab', 'aces_casino_link_mask_slug', 'esc_attr');

    function aces_casino_link_mask_slug_callback($args) {
        $option = get_option( 'aces_casino_link_mask_slug' );
        $id = $args['id'];
        $option_name = $args['option_name'];
        ?>
        <input type="checkbox" name="aces_casino_link_mask_slug" value="1" <?php checked(1, get_option('aces_casino_link_mask_slug'), true); ?> />
        <?php
    }
