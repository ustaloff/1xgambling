...
function aces_games_settings_init() {
    ...
    function aces_games_tab_other_settings_callback( $args ) {
        ?>
        <p id="<?php echo esc_attr( $args['id'] ); ?>">
            <?php esc_html_e( 'Other settings for units.', 'aces' ); ?>
        </p>
        <?php
    }
    ...
    add_settings_field(
        'aces_game_link_mask_slug',
        esc_html__( 'Enable link masking slug', 'aces' ),
        'aces_game_link_mask_slug_callback',
        'aces_games_tab',
        'aces_games_tab_other_settings',
        array(
            'id' => 'aces_game_link_mask_slug', 
            'option_name' => 'aces_game_link_mask_slug'
        )  
    );
    register_setting( 'aces_games_tab', 'aces_game_link_mask_slug', 'esc_attr');

    function aces_game_link_mask_slug_callback($args) {
        $option = get_option( 'aces_game_link_mask_slug' );
        $id = $args['id'];
        $option_name = $args['option_name'];
        ?>
        <input type="checkbox" name="aces_game_link_mask_slug" value="1" <?php checked(1, get_option('aces_game_link_mask_slug'), true); ?> />
        <?php
    }
