...
function aces_bonuses_display_meta_box( $bonus ) {
	...
	if ( get_option( 'aces_bonus_link_mask_slug') ) {
		$bonus_mask_link = get_post_meta( $bonus->ID, 'bonus_mask_link', true );
	}
...
<?php if ( get_option( 'aces_bonus_link_mask_slug') ) { ?>

	<div class="components-base-control bonus_mask_link">
		<div class="components-base-control__field">
			<label class="components-base-control__label" for="bonus_mask_link-0"><?php esc_html_e( 'Custom slug to mask external URL', 'aces' ); ?></label>
			<input type="text" name="bonus_mask_link" id="bonus_mask_link-0" value="<?php echo esc_attr($bonus_mask_link); ?>" style="display: block; margin-bottom: 10px;" />
		</div>
	</div>
...
function aces_bonuses_save_fields( $post_id ) {
        ...
        if ( get_option( 'aces_bonus_link_mask_slug') ) {
        	...
        	$bonus_mask_link = sanitize_text_field( $_POST['bonus_mask_link'] );
        	update_post_meta( $post_id, 'bonus_mask_link', $bonus_mask_link );
