...
$casino_external_link = esc_url( get_post_meta( get_the_ID(), 'casino_external_link', true ) );
	...
	if ( get_option( 'aces_casino_link_mask_slug') && $casino_external_link ) {
		...
		if ( $casino_mask_link_slug ) {
			...
			if ( $_GET['m'] == $casino_mask_link_slug ){
				$redirect_link = esc_url( $casino_external_link );
				header('Location: ' . $redirect_link );
				die();
			}

			$casino_external_link = esc_url( get_the_permalink().'?m='.$casino_mask_link_slug );
						...
						<div class="space-organizations-7-archive-item-buttons-left text-center relative">
							...
							<?php if( function_exists('aces_star_rating') ){ ?>
								<div class="space-organizations-7-archive-item-rating relative">
									...
									<strong><?php echo esc_html( number_format( (float)$overall_rating, 1, '.', ',') ); ?></strong>/<?php echo esc_html( $casino_rating_stars_number_value ); ?>
								</div>
							<?php } ?>
